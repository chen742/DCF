...
def loss_calc(pred, label):
    ...
    gpus=1
    if gpus > 1:
        criterion = torch.nn.DataParallel(CrossEntropy2d(ignore_label=255), device_ids=gpus).cuda()
    else:
...
class HRDAHead(BaseDecodeHead):
    ...
    def __init__(self,
                 ...
                 scales=[1],
                 con_loss_weight=0,
                 source_depth_loss_weight=0,
                 source_correlation_weight=0,
                 target_depth_loss_weight=0,
                 target_correlation_weight=0,
        ...
        self.source_depth_loss_weight = source_depth_loss_weight
        self.source_correlation_weight = source_correlation_weight
        self.target_depth_loss_weight = target_depth_loss_weight
        self.target_correlation_weight = target_correlation_weight
    ...
    def forward_train(self,
        ...
        if stage == 1:
            ...
            # losses['loss_seg'] = losses['loss_seg'] + self.con_loss_weight * loss_contrast

            # corda: source depth loss
            out = feature_outputs['corda']
            ...
            depth = feature_outputs['depth']
            #debug
            # print('init',predD_init)
            # print('final',predD)
            labels = torch.squeeze(gt_semantic_seg, dim=1)
            loss_l_depth = self.source_depth_loss_weight * (loss_depth_calc(predD_init, depth) + loss_depth_calc(predD, depth))
            loss_l_seg = self.source_correlation_weight * loss_calc(pred, labels)
            #print('seg', loss_l_seg)
            #print('depth', loss_l_depth)
            loss_l = loss_l_seg + loss_l_depth
            losses['loss_seg'] = losses['loss_seg'] + loss_l
            #print('total', losses['loss_seg'])


        if stage == 2:
            # corda: target depth loss
            # out = feature_outputs['corda']
            # interp = nn.Upsample(size=(1024, 1024), mode='bilinear', align_corners=True)
            # pred = interp(out["S"])
            # pred_init = interp(out["initial_S"])
            # predD = interp(out["D_src"])
            # predD_init = interp(out["initial_D_src"])
            # depth = feature_outputs['depth_target']
            #
            # labels = torch.squeeze(gt_semantic_seg, dim=1)
            # loss_l_depth = self.target_depth_loss_weight * (loss_depth_calc(predD_init, depth) + loss_depth_calc(predD, depth))
            # loss_l_seg = self.target_correlation_weight * (loss_calc(pred, labels) + loss_calc(pred_init, labels))
            # #print('target_seg', loss_l_seg)
            # #print('target_depth', loss_l_depth)
            # loss_l = loss_l_seg + loss_l_depth
            # losses['loss_seg'] = losses['loss_seg'] + loss_l
            # #print('target_total', losses['loss_seg'])
            pass
        if return_logits:
